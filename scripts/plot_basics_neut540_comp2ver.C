void plot_basics_neut540_comp2ver(){
    gROOT->ProcessLine(".x rootlogon.C"); 
    gROOT->ProcessLine(".L basicPlotUtil.C");
    TString ipDir = "/home/cvson/disk/nuicise/neut/neut_5.4.0/src/neutsmpl/";

    /*TFile *ffile = new TFile(ipDir+"basicplot_5.4.0_600MeV_C_CC1pi_GS.root");
    TString vername = "Graczyk-Sobczyk"; 
     TFile *ffile1 = new TFile(ipDir+"basicplot_5.4.0_600MeV_C_CC1pi_RS.root");
    TString vername1 = "Rein-Seghal";
     TString savename ="neut540_comp2_600mev_cc1pi";*/
 
   TFile *ffile = new TFile(ipDir+"basicplot_5.4.0_600MeV_C_CCcoh_BS.root");
    TString vername = "Berger-Seghal"; 
     TFile *ffile1 = new TFile(ipDir+"basicplot_5.4.0_600MeV_C_CCcoh_RS.root");
    TString vername1 = "Rein-Seghal";
     TString savename ="neut540_comp2_600mev_cccoh";
		...
		//plot2hist(h,vername,h2,vername2,savename+TString(obj->GetName()),0.65,0.65);
		 if(TString(obj->GetName()).Contains("\_q2")) h->GetXaxis()->SetTitle("Q^{2}(GeV^{2})");
                ...
                if(TString(obj->GetName()).Contains("\_XS") ) {
                   plot2hist(h,vername,h1,vername1,savename+TString(obj->GetName()),0.5,0.3);
                }
                else if (TString(obj->GetName()).Contains("TotXS") ) plot2hist(h,vername,h1,vername1,savename+TString(obj->GetName()),0.5,0.3);
                else plot2hist_stat(h,vername,h1,vername1,"",savename+TString(obj->GetName()),false);
