{
    ...
    TFile *pfile;
    pfile = new TFile("basicplotsv7add_antinu_neut540_card_5.4.0_nd5_C_1p1hCCQE_default.root","READ");
    ...
    char *model_name[NCHAN2COMP]={"CC 1#pi all\0","#bar{#nu}_{#mu}+n #rightarrow #mu^{+} + n +#pi^{-}\0" ,"#bar{#nu}_{#mu}+p #rightarrow #mu^{+} + n +#pi^{0}\0", "#nu_{#mu}+p #rightarrow #mu^{+} + p +#pi^{-}\0" };
    ...
    int topoindex[NCHAN2COMP] = {5,6,7,8};//CC1pi
    //no of pion
    ...
    TLatex *l1 = new TLatex(0.3, 0.98, Form("%s,%s GS model",neutversion.Data(),flavor_strlatex[flavorindex]));
    l1->SetNDC();
    l1->SetTextFont(43);
    l1->SetTextSize(20);
    l1->SetTextAlign(13);
    ...
    for ( int ivar = 0 ; ivar < NVAR2PLOT ; ivar++ ){
        ...
        c1->Print(Form("plots/%s_%s_%s_1piGS_pm0.eps",neutversion_s.Data(),flavor_str[flavorindex],hist_type[hist_index[ivar]]));
