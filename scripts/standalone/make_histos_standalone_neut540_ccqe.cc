...
void fill_histos(char *in_fname, char *out_fname){  	
	...
	TH1D *NEUT_pmu        = new TH1D("NEUT_pmu","muon momentum",  100,0.,2.);
   	...
   	TH1D *NEUT_ppro           = new TH1D("NEUT_ppro","proton momentum",  100,0,2);
   	TH1D *NEUT_anglepro       = new TH1D("NEUT_anglepro","proton angle", 180,0,180);
   	TH1D *NEUT_cosanglepro    = new TH1D("NEUT_cosanglepro","cos proton angle", 50,-1.,1.);
	...
	int nproton;
  	for ( Int_t j = 0 ; j < nevents ; j++ ){
		...
		if (nvect->Mode ==1){ // select only CCqe  interaction
