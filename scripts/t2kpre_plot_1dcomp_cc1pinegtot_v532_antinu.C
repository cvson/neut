{
    ...
    pfile[0] = new TFile("basicplotsv8_neut540_antinu_card_5.4.0_nd5_C_1p1hCCQE_default_pineg.root","READ");
    ...
    for ( int ivar = 0 ; ivar < NVAR2PLOT ; ivar++ ){
        ...
        */
        ...
        fHstach[1][ivar]->Draw();
        fHstach[1][ivar]->GetXaxis()->SetTitle(TString(xtitle[ivar]));
        fHstach[1][ivar]->GetYaxis()->SetTitle(ytitle);
        fHstach[1][ivar]->SetMaximum(fHstach[1][ivar]->GetMaximum()*ymaxscale[ivar]);
        fHstach[1][ivar]->SetMinimum(0);
        fHstach[1][ivar]->GetXaxis()->SetRangeUser(xminrange[ivar],xmaxrange[ivar]);
        titleStyle(fHstach[1][ivar]);
        c1->Modified();
