    ...
    for ( nev = 0 ; nev < nevents ; nev++ ){
        ...
        if (abs(nvect->Mode) <30){
            for ( Int_t i = nvect->Nprimary() ; i < nvect->Npart() ; i++ ){
                if(abs((nvect->PartInfo(i))->fPID) == 211 || (nvect->PartInfo(i))->fPID == 111){ // And here, it's a pion, this is awesome !
        ...
        if ( (abs(mode)==1) ){
            ...
            //fill_bindingEvsk_hist(nvtx,h_combined[3][5][ip_idx], rate_norm);//newadd
