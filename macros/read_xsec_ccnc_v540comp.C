...
void read_xsec_ccnc_v540comp(){
    gROOT->ProcessLine(".x rootlogon.C");
    gROOT->ProcessLine(".L basicPlotUtil.C");
    
    TFile *pfile = new TFile("test_neut540kek_gfgccqe.root","READ");
    /*TFile *p2file = new TFile("test_neut540kek_1p1hccqe_default.root","READ");
    TString subname = "v540gfgvs1p1h";*/
    TFile *p2file = new TFile("test_neut540kek_sfccqe.root","READ");
    TString subname = "v540gfgvssf";
    ...
    for (Int_t ichan=1; ichan<NCHAN4SHOWCC; ++ichan) {
        if(ichan!=1)hxsec4show[ichan]->Draw("hist same");//remove 2p2h
    }
    for (Int_t ichan=0; ichan<NCHAN4SHOWCC; ++ichan) {
        if(ichan!=1) h2xsec4show[ichan]->Draw("hist same");//remove 2p2h
    }
    ...
    leg0->SetTextFont(43);
    for (Int_t ichan=0; ichan<NCHAN4SHOWCC; ++ichan) {
        if(ichan!=1)leg0->AddEntry(hxsec4show[ichan],namechan4show[ichan],"l");//remove 2p2h
    }
    leg0->Draw();
    ...
    for (Int_t ichan=0; ichan<NCHAN4SHOWNC; ++ichan) {
        h2xsec4show_nc[ichan]->Draw("hist same");
    }
    //to add legend to the plot
