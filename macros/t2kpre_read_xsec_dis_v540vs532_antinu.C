...
void t2kpre_read_xsec_dis_v540vs532_antinu(){
    ...
    for (Int_t ichan=0; ichan<NCHANNEL; ++ichan) {
        ...
        h2xsec[ichan] = (TH1D*)p2file->Get(Form("neut_xsec_numub_%s",namechan[ichan]));
    ...
    l1->SetTextAlign(33);
    l1->Draw();
    gPad->Print("plots/"+neutversion_s+"_xsec_numub_"+subname+"_cc.eps");
    ...
    l2->SetTextAlign(33);
    l2->Draw();
    gPad->Print("plots/"+neutversion_s+"_xsec_numub_"+subname+"_nc.eps");
