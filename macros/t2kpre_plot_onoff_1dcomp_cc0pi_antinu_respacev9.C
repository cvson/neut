{
    ...
    const Int_t NMODEL2COMP = 4;
    TFile *pfile[NMODEL2COMP];

     
     TString neutversion_s = "onoff_respace_neut";
    bool IsAreaNormalization = true;
    
    pfile[0] = new TFile("basicplotsv9_neut540_antinu_card_5.4.0_nd2_C_1p1hCCQE_default_cc0pirespace.root","READ");
    pfile[1] = new TFile("basicplotsv9_neut540_antinu_card_5.4.0_nd5_C_1p1hCCQE_default_cc0pirespace.root","READ");
    pfile[2] = new TFile("basicplotsv9_neut532_antinu_card_5.3.6_nd2_C_GFGCCQErpa_cc0pirespace.root","READ");
    pfile[3] = new TFile("basicplotsv9_neut532_antinu_card_5.3.6_nd5_C_GFGCCQErpa_cc0pirespace.root","READ");
    
    const int NFLAVOR = 6;
    const int NHIST1D = 38;
    const int NCHANNEL = 11;//12?
    const int NHIST2D = 8;
    int topoindex = 5;//CC0pi topo
    
    char *model_name[NMODEL2COMP]={"On-axis, #bar{#nu}_{#mu} v5.4.0" ,"Off-axis, #bar{#nu}_{#mu} v5.4.0","On-axis, #bar{#nu}_{#mu} v5.3.2" ,"Off-axis, #bar{#nu}_{#mu} v5.3.2"};
    ...
    //basic_histos_v9topobar_resphace.cc
    //phasesphace restric
    ...
    ylegmin[1]= 0.65;
    ymaxscale[1]= 1.511;
    if(topoindex==9 && IsAreaNormalization) ymaxscale[1]= 2.011;
    ...
    ylegmin[2]= 0.65;
    ymaxscale[2]= 3.011;
    if((!IsAreaNormalization) || topoindex==9 || topoindex==5) ymaxscale[2]= 1.512;
    ...
    xtitle[8]= "Highest energy proton P_{n} (GeV)";
    xminrange[8]= 0;
    xmaxrange[8]= 2;//
    xlegmin[8]= 0.55;
    ylegmin[8]= 0.45;
    ymaxscale[8]= 1.511;
        if(topoindex==9 && IsAreaNormalization) ymaxscale[8]= 2.011;
    ...
    hist_index[13]= 34;//highest energy proton angle momentum
    xtitle[13]= "Highest energy proton cos #theta_{n}";
    xminrange[13]= -1;
    xmaxrange[13]= 1;//
    xlegmin[13]= 0.25;
    ylegmin[13]= 0.45;
    ...
    for ( int ifile = 0 ; ifile < NMODEL2COMP ; ifile++ ){//change to 11
        for ( int ivar = 0 ; ivar < NVAR2PLOT ; ivar++ ){
            ...
            if(ifile>1)h_1d_comp[ifile][ivar]->SetLineStyle(3);
        }
    }
    ...
    for ( int ivar = 0 ; ivar < NVAR2PLOT ; ivar++ ){
        ...
        if(IsAreaNormalization) h_1d_comp[0][ivar]->GetYaxis()->SetTitle("Area normalization");
        else h_1d_comp[0][ivar]->GetYaxis()->SetTitle(ytitle);
        ...
        double area1 = h_1d_comp[0][ivar]->Integral();
        for ( int ifile = 1 ; ifile < NMODEL2COMP ; ifile++ ){//change to 11
            //area normalization
            double area2 =h_1d_comp[ifile][ivar]->Integral();
            if(IsAreaNormalization) h_1d_comp[ifile][ivar]->Scale(area1/area2);
        ...
        h_1d_comprat[0][ivar]->GetYaxis()->SetTitle("MC/on-axis v5.4.0");
        ...
        if(IsAreaNormalization) c1->Print(Form("plots/%s_%s_%s_%s_areanom.eps",neutversion_s.Data(),flavor_str[flavorindex],hist_type[hist_index[ivar]],hist_kind[topoindex]));
        
        else c1->Print(Form("plots/%s_%s_%s_%s.eps",neutversion_s.Data(),flavor_str[flavorindex],hist_type[hist_index[ivar]],hist_kind[topoindex]));
