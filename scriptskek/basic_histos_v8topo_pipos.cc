...
basic_histos(char *fname,char *fname_out)
{
	...
	npionany =0;//new 20180731
        ...
        if (abs(nvect->Mode) <30){
            ...
            for ( Int_t i = 2 ; i < nvect->Npart() ; i++ ){
                if ((nvect->PartInfo(i))->fIsAlive){
                    ...
                    if((nvect->PartInfo(i))->fPID == 211){//pi minus for nu run
			    npion +=1;//check number of pion
                    }
		    if(abs((nvect->PartInfo(i))->fPID) == 211 || (nvect->PartInfo(i))->fPID == 111){npionany +=1;}
        ...
        if ( isCCtopo0pi(npionany,mode) ){//change on 20180731
