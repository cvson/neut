...
const int NFLAVOR = 6;
const int NHIST1D = 38;//34 in version 8
...
fill_rescat_pnuc1_hist(NeutVect *nvect, TH1D *h1, Double_t weight)
{
    ...
    //this is a bug
    if ( mode == 2 ){
        //idx = 4;//this is wrong 20180416
        first_idx = 4;//
    }
    //e1 = 0;//this is not correct
    e1 = (nvect->PartInfo(first_idx))->fP.E();
    idx = first_idx;
    for ( i = first_idx; i < nvect->Npart(); i++ ){
        if ((nvect->PartInfo(i))->fIsAlive){
            ip = (nvect->PartInfo(i))->fPID;// this is another bug, 20180416
            //if (( ip == 2212 ) || ( ip == 2112 )){// // why neutron here? we want proton right? wrong in anti-neutrino?
            if ( ip == 2212 ){//look at proton only
                e2 = (nvect->PartInfo(i))->fP.E();
                if ( e2 > e1 ){// to get highest proton energy
                    e1 = e2;
                    idx = i;
                }
            }
...
//2 highest proton: momentum 1st, angle 1st, momentum
Int_t
fill_rescat_nucff_hist(NeutVect *nvect, TH1D *h1, TH1D *h2, TH1D *h3, TH1D *h4, TH1D *h5, Double_t weight)
{
    //idx is index for highest energy and idxno2 is 2nd highest energy
    //h1, h2 is momentum, angle of highest energy proton
    //h3, h4 is momentum, angle of 2nd highest energy proton
    //h5 is opening angle of two highest proton
    Int_t    first_idx,ip,i,idx,idxno2;
    Int_t    mode;
    
    Double_t e1,e2,e12;//to store the 2nd energy
    first_idx = 3;
    mode = abs(nvect->Mode);
    if (( mode == 16 ) || ( mode == 36 )){
        return 0;
    ...
    //e1 = (nvect->PartInfo(first_idx))->fP.E();// this should cause a bug
    idx = first_idx;
    idxno2 = -1;//2nd proton
    
    //search 1st highest track
    for ( i = first_idx; i < nvect->Npart(); i++ ){
        if ((nvect->PartInfo(i))->fIsAlive){
            ip = (nvect->PartInfo(i))->fPID;// this is another bug, 20180416
            if ( ip == 2212 ){//look at proton only
                e2 = (nvect->PartInfo(i))->fP.P();//E or P?
                if ( e2 > e1 ){// to get highest proton energy
                    e1 = e2;
                    idx = i;// 1st highest
                }
    ...
    e12 =0;
    //search 2nd highest track
    for ( i = first_idx; i < nvect->Npart(); i++ ){
        if ((nvect->PartInfo(i))->fIsAlive && i!=idx){ //alive and not first track
            ip = (nvect->PartInfo(i))->fPID;// this is another bug, 20180416
            if ( ip == 2212 ){//look at proton only
                e2 = (nvect->PartInfo(i))->fP.P();//E or P?
                if ( e2 > e12 ){// to get highest proton energy
                    e12 = e2;
                    idxno2 = i;//2nd highest
                }
    ...
    fill_momentum(nvect, idx, h1, weight);
    fill_costhetaL(nvect, idx, h2, weight);
    //only fill 2nd if different from the 1st
    if(idxno2!=idx && idxno2>0){
        fill_momentum(nvect, idxno2, h3, weight);
        fill_costhetaL(nvect, idxno2, h4, weight);
        fill_opening_angle(nvect,idx,idxno2,h5,weight);//opening angle
    }
                ...
                double xnorm)
{
    ...
    //27 initial pion momentum
    //28 initial pion cos angle
    //29 final pion momentum
    //30 final pion cos angle
    //31 different in pion momentum final-inial
    //32 different in pion angle final-initial
    //33 reconstructed W
    //34 1st highest proton angle
    //35 2nd highest proton momentum
    //36 2nd highest proton angle
    //37 opening angle between 1st and 2nd highest proton
    ...
    fill_rescat_nucff_hist(nvect, h[idx][16][ip_idx], h[idx][34][ip_idx],h[idx][35][ip_idx],h[idx][36][ip_idx],h[idx][37][ip_idx],xnorm);//newadd 20180418
...
basic_histos(char *fname,char *fname_out)
{
    ...
    char hist_type[NHIST1D][16]={"enu\0"          ,"plep\0"       ,"in_nuc1_p\0",
        ...
        "iniPiP\0","iniPicos\0","finPiP\0","finPicos\0","finPiPdiff\0","finPicosdiff\0","wres\0",
        "rescat_nuc1_a\0", "rescat_nuc2_p\0", "rescat_nuc2_a\0", "rescat_opening\0"//newadd 20180418
    };
    ...
    int nev,iflv,ip,ip_idx,mode, npion, nproton;
    int nproton250mev, nproton300mev, nproton350mev;
    const int flavor_id[NFLAVOR]={12,14,16,-12,-14,-16};
    
    Int_t    leptonidx,leptonip;//for lepton information
    Int_t   profirst_idx, proidx, proip;
    double proe1, proe2;//to get proton with highest energy
    for ( nev = 0 ; nev < nevents ; nev++ ){
        ...
        nproton350mev = 0;
        /////////////////////////////////////////////////
        ///To get information for phase space cut, need muon momentum, angle; proton momentum, angle
        double leptonmon = 0;
        double leptoncosangle = 0;
        double protonmon = 0;
        ...
        if (abs(nvect->Mode) <30){
            //----------get lepton momentum angle
            leptonidx = 2;
            if ( abs(mode) == 2 ){
                leptonidx = 3;
            }
            leptonip = abs((nvect->PartInfo(leptonidx))->fPID);
            if (isLepton(leptonip)){
                leptoncosangle = (nvect->PartInfo(0))->fP.Vect()*(nvect->PartInfo(leptonidx))->fP.Vect();
                leptoncosangle = leptoncosangle /
                ((nvect->PartInfo(0))->fP.P()*(nvect->PartInfo(leptonidx))->fP.P());
                leptonmon = nvect->PartInfo(leptonidx)->fP.P();
                leptonmon = leptonmon/1000.;
            }
            //-----------end getting lepton momentum angle
            
            //----------Get proton momentum, angle
            profirst_idx = 3;
            if ( abs(mode) == 2 ){
                profirst_idx = 4;
            }
            
            proe1 = (nvect->PartInfo(profirst_idx))->fP.E();
            proidx = profirst_idx;
            for (Int_t i = profirst_idx; i < nvect->Npart(); i++ ){
                if ((nvect->PartInfo(i))->fIsAlive){
                    proip = (nvect->PartInfo(i))->fPID;
                    //if ( proip == 2212 || proip == 2112 ){//here why neutron here? we want proton right?
                    if ( proip == 2212 ){// only proton
                        proe2 = (nvect->PartInfo(i))->fP.E();
                        if ( proe2 > proe1 ){
                            proe1 = proe2;
                            proidx = i;
                        }
            ...
            protonmon = (nvect->PartInfo(proidx))->fP.P();
            protonmon = protonmon / 1000.;
            //-----------end proton momentum, angle
            
            //for ( Int_t i = nvect->Nprimary() ; i < nvect->Npart() ; i++ ){
            for ( Int_t i = 3 ; i < nvect->Npart() ; i++ ){//2 here seems wrong; should be 3
                if ((nvect->PartInfo(i))->fIsAlive){
                    ...
                    if((nvect->PartInfo(i))->fPID == 2212){ // And here, it's a proton, this is awesome !
                        //only select with proton threshold
                        //if((nvect->PartInfo(i))->fPID == 2212 && (nvect->PartInfo(i))->fP.P()>300){
                        nproton +=1;//check number of proton
                        if ((nvect->PartInfo(i))->fP.P()>250) nproton250mev +=1;
                        if ((nvect->PartInfo(i))->fP.P()>300) nproton300mev +=1;
                        if ((nvect->PartInfo(i))->fP.P()>350) nproton350mev +=1;
                    }
                }
            }
        ...
        if ( (abs(mode)==2) && nproton250mev==2  ){
            ...
            fill_nproton_hist(h_combined[6][21][ip_idx],rate_norm,nproton);
            fill_npion_hist(h_combined[6][22][ip_idx],rate_norm,npion);
            fill_out_ppiINI_hist(nvect,h_combined[6][27][ip_idx],h_combined[6][28][ip_idx],h_2d_combined[6][6][ip_idx],rate_norm);
            fill_out_ppiFSI_hist(nvect,h_combined[6][29][ip_idx],h_combined[6][30][ip_idx],h_combined[6][31][ip_idx],h_combined[6][32][ip_idx],h_2d_combined[6][7][ip_idx],rate_norm);
        }
        //add cut on lepton momentum
        if ( isCC(mode) && nproton300mev==2 ){
            ...
            fill_out_ppiINI_hist(nvect,h_combined[7][27][ip_idx],h_combined[7][28][ip_idx],h_2d_combined[7][6][ip_idx],rate_norm);
            fill_out_ppiFSI_hist(nvect,h_combined[7][29][ip_idx],h_combined[7][30][ip_idx],h_combined[7][31][ip_idx],h_combined[7][32][ip_idx],h_2d_combined[7][7][ip_idx],rate_norm);
        }
        //add cut on lepton angle TMath::Cos(50*TMath::Pi()/180.)
        //(Double_t)6.42787609686539363e-01
        if ( (abs(mode)==1) && nproton300mev==2){
        ...
        if ( isCCtopo0pi(npion,mode) && protonmon>0.450){
